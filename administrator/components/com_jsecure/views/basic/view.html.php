...
class jsecureViewBasic extends JViewLegacy {
	protected $form;
	protected $item;
	protected $state;
	...
	//Log start here
		...
		$captcha = $model->getCaptchaKey();	
		$captchaDataSiteKey = $model->getCaptchaDataSiteKey();
		
		// Create the pagination object
		$this->assignref('captcha',$captcha);
		$this->assignref('captchadatasitekey',$captchaDataSiteKey);
    ...
    protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('jSecure Authentication'), 'generic.png');
			...
			JToolBarHelper::apply('applyBasic');
	...
	function save(){
 	...
 	function saveDetails(){	
		...
		$config_array['captchapublish']	             = JRequest::getVar('captchapublish', 0, 'post', 'int');
...
//		$modifiedFieldName	=$this->checkModifiedFieldName($config_array, $oldValue, $JSecureCommon, $keyvalue, $masterkeyvalue);
		...
		/*---Code to insert captcha key in database----*/
		
		
		$model 	= $this->getModel( 'jsecurelog' );
		
		$msg = JText::_('CAPTCHA_KEY');
		
							
		$insertCaptchaKey = $model->insertCaptchaKey($msg,$post['captchakey']);
				
		/*---Code to insert captcha key in database----*/
		
		/*---Code to insert captcha data site key in database----*/
		
		
		$model 	= $this->getModel( 'jsecurelog' );
		
		$msg = JText::_('CAPTCHA_DATA_SITE_KEY');
		
							
		$insertCaptchaDataSiteKey = $model->insertCaptchaDataSiteKey($msg,$post['captchadatasitekey']);
