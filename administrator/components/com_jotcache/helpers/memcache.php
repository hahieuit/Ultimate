...
$list = array();
$allSlabs = self::$db->getExtendedStats('slabs');
    foreach ($allSlabs as $server => $slabs) {
foreach ($slabs AS $slabId => $slabMeta) {
$cdump = self::$db->getExtendedStats('cachedump', (int) $slabId);
foreach ($cdump AS $keys => $arrVal) {
if (!is_array($arrVal))
continue;
foreach ($arrVal AS $k => $v) {
if (strlen($k) == 48) {
if (substr($k, 0, 16) == self::$lead) {
$list[] = substr($k, 16);
}}}}}}return $list;
}public function store($data = null) {
if ($data) {
$cacheId = self::$lead . $this->fname;
if (!self::$db->replace($cacheId, $data, $this->compress, $this->_lifetime)) {
self::$db->set($cacheId, $data, $this->compress, $this->_lifetime);
}}return true;
