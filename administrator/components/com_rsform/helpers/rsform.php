...
{	
	public function isJ16()
	{
		// just for legacy reasons
		return true;
	}
	
	public function isJ($version) {
		static $cache = array();
	...
	public function getLegacyAdapter()
	{
		static $adapter;
		if (empty($adapter))
		{
	...
	public function getComponentId($name, $formId=0)
	{
		static $cache;
		if (!is_array($cache))
			$cache = array();
	...
	public function checkValue($setvalue, $array)
	{
		if (!is_array($array))
	...
	public function createList($results, $value='value', $text='text')
	{
		$list = array();
		if (is_array($results))
	...
	public function displayForm($formId, $is_module=false)
	{
		...
		if (isset($formparams) && $formparams->formProcessed)
		{
		...
		{
			if($formId==5) { // Quick Form on product page
				$session = JFactory::getSession();
				$session->set('f'.$formId.'Name', $post['Name']);
				$session->set('f'.$formId.'Company', $post['Company']);
				$session->set('f'.$formId.'Phone', $post['Phone']);
				$session->set('f'.$formId.'Delivery', $post['Delivery']);
				$session->set('f'.$formId.'Email', $post['Email']);
			}
			...
			{
				if($formId==5) { // Quick Form on product page
					$session->set('f'.$formId.'Name','');
					$session->set('f'.$formId.'Company', '');
					$session->set('f'.$formId.'Phone', '');
					$session->set('f'.$formId.'Delivery', '');
					$session->set('f'.$formId.'Email', '');
				}
		...
		if (isset($formparams) && $formparams->formProcessed)
		{
			...
			//strip_tags(base64_decode($formparams->thankYouMessage));
            return "<script> alert('". $message."');</script>".
			RSFormProHelper::showForm($formId).
			'
<div style="display:none;">
<img height="1" width="1" style="border-style:none;width:1px;height:1px" alt=""  
src="//www.googleadservices.com/pagead/conversion/1021672865/?value=0&amp;label=sjleCO_JqQgQofuV5wM&amp;guid=ON&amp;script=0"/>
</div>'
			;
		}
		else{
	...
	public function WYSIWYG($name, $content, $hiddenField, $width, $height, $col, $row)
    {
	...
	public function getOtherCalendars() {
		$db 	= JFactory::getDbo();
		$list 	= array();
	...
	public function getDateValidationRules() {
	...
	public function componentNameExists($componentName, $formId, $currentComponentId=0)
	{
		$db = JFactory::getDBO();
	...
	public function getCurrentLanguage($formId=null)
	{
		$mainframe = JFactory::getApplication();
		$lang 	   = JFactory::getLanguage();
	...
	public function getComponentProperties($components)
	{
		$db = JFactory::getDBO();
	...
	public function showPreview($formId, $componentId, $data)
	{
		$mainframe = JFactory::getApplication();
	...
	public function readFile($file, $download_name=null)
	{
	...
	public function getReplacements($SubmissionId, $skip_globals=false)
	{
		// Small hack
		return RSFormProHelper::sendSubmissionEmails($SubmissionId, true, $skip_globals);
	}
	
	public function sendSubmissionEmails($SubmissionId, $only_return_replacements=false, $skip_globals=false)
	{
		$db = JFactory::getDBO();
		$u = JUri::getInstance();
	...
	public function escapeArray(&$val, &$key)
	{
		$db = JFactory::getDBO();
		$val = $db->escape($val);
		$key = $db->escape($key);
	...
	public function componentExists($formId, $componentTypeId)
	{
		$formId = (int) $formId;
		$db = JFactory::getDBO();
	...
	public function loadTheme($form)
	{
	...
	// conditions
	public function getConditions($formId, $lang=null)
	{
		$db   = JFactory::getDBO();
	...
	public function showForm($formId, $val=array(), $validation=array())
	{
		$mainframe = JFactory::getApplication();
	...
	public function showThankYouMessage($formId)
	{
		$mainframe = JFactory::getApplication();
	...
	public function verifyChecked($componentName, $value, $post)
	{
	...
	public function getFrontComponentBody($formId, $componentId, $data, $value=array(), $invalid=false, $layoutName)
	{
		$mainframe = JFactory::getApplication();
	...
	public function addClass(&$attributes, $className)
	{
	...
	public function addOnClick(&$attributes, $onClick)
	{
	...
	public function getInvisibleCaptchaWords()
	{
		return array('Website', 'Email', 'Name', 'Address', 'User', 'Username', 'Comment', 'Message');
	}
	
	public function generateString($length, $characters, $type='Random')
	{
		$length = (int) $length;
		if($type == 'Random')
		{
	...
	public function getTranslations($reference, $formId, $lang, $select = 'value')
	{		
		$db = JFactory::getDBO();
	...
	public function mappingsColumns($config,$method,$row = null)
	{
	...
	public function getMappingQuery($row)
	{
		$db = JFactory::getDBO();
		$query = '';
